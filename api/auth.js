  ...
  function(req,res,next){
    ...
    .exec(function(err,user){
      if(err) return res.json(util.successFalse(err));
        ...
        var payload = {         // token에 저장될 정보
          _id : user._id,
          username: user.username
        };
        var secretOrPrivateKey = process.env.JWT_SECRET;    //hash 생성에 사용되는 key문자열
...
// me :  token을 해독해서 DB에서 user 정보를 return하는 API
...
// refresh : refresh함수는 token의 유효기간이 끝나기전에 새로운 토큰을 발행하는 API
